...
if __name__ == "__main__":
    ...
    for sample in tqdm(list_data_dict):
        ...
        if not "On what date was the Declaration of Independence officially signed?" in input_text:
            continue
        with torch.no_grad():
            ...
            # outputs = llm.model.generate(input_ids, dola_layers=candidate_premature_layers, max_new_tokens=50, repetition_penalty=args.repetition_penalty, 
            #                             num_return_sequences=1,
            #                             output_scores=True, return_dict_in_generate=True, )
            # # model_completion, c_dist = llm.generate(input_text, **generate_kwargs)
            # # c_dist = None #outputs.premature_layer_dist
            ...
            # output_str = llm.tokenizer.decode(gen_sequences, skip_special_tokens=True)
            # torch.cuda.empty_cache()
            generate_kwargs_vanilla = dict(max_new_tokens=args.max_new_tokens, top_p=args.top_p, top_k=args.top_k, temperature=args.temperature, repetition_penalty=args.repetition_penalty, mode='baseline') #, candidate_premature_layers=candidate_premature_layers, mature_layer=mature_layer, premature_layer=premature_layer, relative_top=args.relative_top)
            model_completion_vanilla, c_dist = llm.generate(input_text, **generate_kwargs_vanilla)
            generate_kwargs_dola = dict(max_new_tokens=args.max_new_tokens, top_p=args.top_p, top_k=args.top_k, temperature=args.temperature, repetition_penalty=args.repetition_penalty, mode=mode, candidate_premature_layers=candidate_premature_layers, mature_layer=mature_layer, premature_layer=premature_layer, relative_top=args.relative_top)
            model_completion_dola, c_dist = llm.generate(input_text, **generate_kwargs_dola)

        print(f'Vanilla: {model_completion_vanilla}\n\n')
        print(f'DoLa: {model_completion_dola}\n\n')
        import ipdb; ipdb.set_trace()
        # model_completion = output_str
